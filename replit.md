...
- `artifacts/api-server/src/routes/sites.ts` — `GET /api/sites` (user's sites) + `POST /api/sites/claim-legacy` (old admin password, timing-safe SHA-256 compare, rate-limited, single-claim via conditional UPDATE); the escalating-lockout upsert lives in `lib/claimRateLimit.ts` (integration-tested against the dev DB)
