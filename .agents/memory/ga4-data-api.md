...
**Default geo exclusion (2026-07-27, user directive):** ALL GA4 reports exclude Pakistan — `notExpression` on `countryId` EXACT `"PK"` (ISO code, not the "country" display name), composed into every runReport's dimensionFilter via the shared `ga4DimensionFilter()` helper in the GA4 integration module. Any NEW runReport added later must go through that helper or it will silently re-include PK traffic. This applies to the host-unfiltered key-events reports too (host and geo filters are orthogonal). Verified exact: filtered total = baseline − PK row. Remember withCache keys must bump whenever report semantics change.
