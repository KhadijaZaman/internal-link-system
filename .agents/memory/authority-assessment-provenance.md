...
- **Dashboard linkage:** the Linkweave sidebar's "Site Authority" is the internal embedding-snapshot tool at SPA route `/authority` — NOT this report. The report is linked separately as "Authority Report" (admin-gated `<a target=_blank>` to `/authority/`, kept hidden from other tenants because the report is Wellows-specific). Known pre-existing collision: full page loads of `/authority` in prod hit the report artifact, shadowing the SPA snapshot page on refresh/deep-link; fix would be renaming the SPA route (e.g. `/site-authority`).
