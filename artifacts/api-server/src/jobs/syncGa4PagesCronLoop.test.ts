...
/**
 * Unit tests for the per-site cron loop that drives sync_ga4_pages.
 *
 * runJobForAllSites iterates every claimed site sequentially. A GA4 Data API
 * failure for one site must be logged and isolated — it must not abort the
 * remaining sites. These tests verify that guarantee at the unit level (no
 * real DB, no real GA4 API) by mocking the two collaborators that the loop
 * depends on: listSchedulableSites and runJob.
 *
...
describe("sync_ga4_pages cron loop — per-site error isolation", () => {
  it("continues to remaining sites when one site's completion rejects with a GA4 API error", async () => {
    // Site 2 simulates a GA4 Data API failure (e.g. quota exceeded, token
    // revoked, network blip). The completion promise rejects — this is the
    ...
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 2) {
        return {
          started: true,
          completion: Promise.reject(new Error("GA4 Data API quota exceeded")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });

    await runJobForAllSites("sync_ga4_pages");
  ...
  it("logs the failure for the failing site with siteId context", async () => {
    ...
    expect(meta).toMatchObject({ siteId: 2, jobName: "sync_ga4_pages" });
  ...
  it("does not propagate the error — runJobForAllSites always resolves", async () => {
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 1) {
        return {
          started: true,
          completion: Promise.reject(new Error("GA4 service account auth revoked")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
    ...
    await expect(runJobForAllSites("sync_ga4_pages")).resolves.toBeUndefined();
  });
  ...
  it("runs subsequent sites even when the first site fails", async () => {
    ...
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      ...
      if (site.id === 1) {
        return {
          started: true,
          completion: Promise.reject(new Error("forced first-site GA4 failure")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
