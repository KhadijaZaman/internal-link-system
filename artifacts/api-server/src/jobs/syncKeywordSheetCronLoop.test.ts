...
/**
 * Unit tests for the per-site cron loop that drives sync_keyword_sheet.
 *
...
describe("sync_keyword_sheet cron loop — per-site error isolation", () => {
  it("continues to remaining sites when one site's completion rejects with a Sheets API error", async () => {
    ...
    await runJobForAllSites("sync_keyword_sheet");
  ...
  it("logs the failure for the failing site with siteId context", async () => {
    ...
    expect(meta).toMatchObject({ siteId: 2, jobName: "sync_keyword_sheet" });
  ...
  it("does not propagate the error — runJobForAllSites always resolves", async () => {
    ...
    await expect(runJobForAllSites("sync_keyword_sheet")).resolves.toBeUndefined();
  });
