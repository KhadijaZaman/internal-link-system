...
/**
 * Unit tests for the per-site cron loop that drives sync_link_map_sheet.
 *
 * runJobForAllSites iterates every claimed site sequentially. A Sheets API
 ...
 * (no real DB, no real Sheets API) by mocking the two collaborators that
...
describe("sync_link_map_sheet cron loop — per-site error isolation", () => {
  it("continues to remaining sites when one site's completion rejects with a Sheets API error", async () => {
    // Site 2 simulates a Sheets API failure (e.g. 429, auth revoked, network blip).
    ...
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 2) {
        return {
          started: true,
          completion: Promise.reject(new Error("Sheets API 429 Too Many Requests")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });

    await runJobForAllSites("sync_link_map_sheet");
  ...
  it("logs the failure for the failing site with siteId context", async () => {
    ...
    expect(meta).toMatchObject({ siteId: 2, jobName: "sync_link_map_sheet" });
  ...
  it("does not propagate the error — runJobForAllSites always resolves", async () => {
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 1) {
        return {
          started: true,
          completion: Promise.reject(new Error("Sheets API auth revoked")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
    ...
    await expect(runJobForAllSites("sync_link_map_sheet")).resolves.toBeUndefined();
  });
  ...
  it("runs subsequent sites even when the first site fails", async () => {
    ...
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      ...
      if (site.id === 1) {
        return {
          started: true,
          completion: Promise.reject(new Error("forced first-site failure")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
