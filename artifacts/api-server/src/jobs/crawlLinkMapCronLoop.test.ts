...
/**
 * Unit tests for the per-site cron loop that drives crawl_link_map.
 *
 * runJobForAllSites iterates every claimed site sequentially. A crawl failure
 * (network timeout, robots.txt block, sitemap parse error) for one site must
 * be logged and isolated — it must not abort the remaining sites. These tests
...
describe("crawl_link_map cron loop — per-site error isolation", () => {
  it("continues to remaining sites when one site's crawl rejects with a network timeout", async () => {
    // Site 2 simulates a network timeout during the sitemap fetch or page
    // crawl. The completion promise rejects — this is the path the defensive
    // catch in runJobForAllSites is designed to absorb.
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 2) {
        return {
          started: true,
          completion: Promise.reject(new Error("Crawl: network timeout fetching sitemap")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });

    await runJobForAllSites("crawl_link_map");
  ...
  it("logs the failure for the failing site with siteId context", async () => {
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 2) {
        return {
          started: true,
          completion: Promise.reject(new Error("Sitemap fetch failed: 403")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
    ...
    expect(meta).toMatchObject({ siteId: 2, jobName: "crawl_link_map" });
  ...
  it("does not propagate the error — runJobForAllSites always resolves", async () => {
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 1) {
        return {
          started: true,
          completion: Promise.reject(new Error("Crawl: robots.txt disallows all paths")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });

    // Must resolve, never reject, even when a site's crawl throws.
    await expect(runJobForAllSites("crawl_link_map")).resolves.toBeUndefined();
  });
  ...
  it("runs subsequent sites even when the first site fails", async () => {
    ...
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      ...
      if (site.id === 1) {
        return {
          started: true,
          completion: Promise.reject(new Error("Child sitemap site-one.example/sitemap-posts.xml failed")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
  ...
  it("handles the case where runJob itself throws (defensive catch path)", async () => {
    ...
    // e.g. the runner crashes before it can even start the crawl.
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 2) {
  ...
  it("isolates parse errors — a malformed sitemap on one site does not stop others", async () => {
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 3) {
        return {
          started: true,
          completion: Promise.reject(new Error("Crawl: sitemap XML parse error")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
    ...
    expect(calledSiteIds).toEqual([1, 2, 3]);
    expect(loggerErrorMock).toHaveBeenCalledOnce();
    expect(loggerErrorMock.mock.calls[0]![0]).toMatchObject({ siteId: 3, jobName: "crawl_link_map" });
  });
});
