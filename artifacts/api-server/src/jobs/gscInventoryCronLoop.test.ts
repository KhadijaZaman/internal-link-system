...
/**
 * Unit tests for the per-site cron loop that drives gsc_inventory_and_losers.
 *
 * runJobForAllSites iterates every claimed site sequentially. A GSC API
 * failure for one site must be logged and isolated — it must not abort the
 * remaining sites. These tests verify that guarantee at the unit level
 * (no real DB, no real GSC API) by mocking the two collaborators that
 * the loop depends on: listSchedulableSites and runJob.
 *
...
describe("gsc_inventory_and_losers cron loop — per-site error isolation", () => {
  it("continues to remaining sites when one site's completion rejects with a GSC API error", async () => {
    // Site 2 simulates a GSC API failure (e.g. 403, quota exceeded, network blip).
    ...
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 2) {
        return {
          started: true,
          completion: Promise.reject(new Error("GSC API 403 Forbidden")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });

    await runJobForAllSites("gsc_inventory_and_losers");
  ...
  it("logs the failure for the failing site with siteId context", async () => {
    ...
    expect(meta).toMatchObject({ siteId: 2, jobName: "gsc_inventory_and_losers" });
  ...
  it("does not propagate the error — runJobForAllSites always resolves", async () => {
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      if (site.id === 1) {
        return {
          started: true,
          completion: Promise.reject(new Error("GSC API token revoked")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
    ...
    await expect(runJobForAllSites("gsc_inventory_and_losers")).resolves.toBeUndefined();
  });
  ...
  it("runs subsequent sites even when the first site fails", async () => {
    ...
    runJobMock.mockImplementation(async (_name: string, site: { id: number }) => {
      ...
      if (site.id === 1) {
        return {
          started: true,
          completion: Promise.reject(new Error("forced first-site GSC failure")),
        };
      }
      return { started: true, completion: Promise.resolve() };
    });
